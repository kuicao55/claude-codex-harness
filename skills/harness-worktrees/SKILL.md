...
- At the start of `harness-execution` (Orchestrator will call this)
- When `harness-brainstorming` or `harness-plan-writing` ends and the user is about to begin execution
...
Provide the worktree path to `harness-execution` as the working directory for all Executor subagents.

---

## Worktree Cleanup (After Finishing)

This is handled by `harness-finishing`. But if manual cleanup is needed:
...
When `harness-parallel-dispatch` is used for parallel Executor instances:
