...
- Before marking a task complete in `harness-execution`
...
If any item fails, STOP. Do NOT claim completion. Invoke `harness-debugging` if needed.
...
This skill is invoked by `harness-execution` before:

- Marking a milestone as passed
- Claiming all tasks are complete
- Invoking `harness-finishing`
