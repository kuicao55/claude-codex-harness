...
Invoke the `harness-entry` skill with resume mode.

Tell the user: "Resuming harness session. Loading handoff document to find where we left off..."

Then immediately read and follow the `harness-entry` skill, passing the context that this is a `/super-harness:resume` invocation.
