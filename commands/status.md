...
1. Look for `.super-harness/status/claude-progress.json` in the current working directory.
2. If the file does not exist: Tell the user "No progress file found at `.super-harness/status/claude-progress.json`. This project may be a small single-session project (no progress tracking) or has not been started yet with `/super-harness:brainstorm` or `/super-harness:plan`."
