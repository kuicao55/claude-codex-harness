...
2. If the file does not exist: Tell the user "No progress file found at `status/claude-progress.json`. This project may be a small single-session project (no progress tracking) or has not been started yet with `/harness:brainstorm` or `/harness:plan`."
...
4. After displaying, offer: "Would you like to `/harness:resume` the current milestone, or is there anything else you'd like to do?"
